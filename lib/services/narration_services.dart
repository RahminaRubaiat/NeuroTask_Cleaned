...
class NarrationServices {

  static Future<void> sendAudioToDatabase (String filePath) async{
  try {
    final FirebaseStorage storage = FirebaseStorage.instance;
    final Reference ref = storage.ref().child('videos/Narration-1003-$patientemail-${DateTime.now()}.aac'); // Use a unique filename
    final UploadTask uploadTask = ref.putFile(File(filePath));

    final TaskSnapshot snapshot = await uploadTask;
    final String downloadUrl = await snapshot.ref.getDownloadURL();
    FirebaseFirestore.instance.collection("Narration - 1003 - $patientemail").doc(DateTime.now().toString()).set({
      'game_id' : '1003',
